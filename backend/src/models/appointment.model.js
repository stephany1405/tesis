  ...
  try {
    ...
    for (let servicio of orderData.servicios) {
      ...
      if (serviceResult.rowCount === 0) {
        throw new Error(`No se encontró el servicio para ${servicio.classification_type}`);
      }
    ...
    const appointmentPromises = servicesData.map(async ({ service_id, note_of_service }) => {
      const query = {
